...
if (isset($_SESSION[$settings['session_prefix'].'user_id']))
 {
  ...
  if(isset($_GET['edit']))
   {
    $dbr = Database::$content->prepare("SELECT id, title, teaser, text, text_formatting, link, linkname FROM ".Database::$db_settings['notes_table']." WHERE note_section=:note_section ORDER BY sequence ASC");
  ...
  if(isset($_GET['edit_note']))
   {
    $dbr = Database::$content->prepare("SELECT id, note_section, time, title, teaser, text, text_formatting, link, linkname FROM ".Database::$db_settings['notes_table']." WHERE id=:id LIMIT 1");
    ...
    if(empty($errors))
     {
      if(isset($_POST['id']))
       {
        $dbr = Database::$content->prepare("UPDATE ".Database::$db_settings['notes_table']." SET time=:time, title=:title, teaser=:teaser, text=:text, text_formatting=:text_formatting, link=:link, linkname=:linkname WHERE id=:id");
      ...
      else
       {
        ...
        $dbr = Database::$content->prepare("INSERT INTO ".Database::$db_settings['notes_table']." (note_section,sequence,time,title,teaser,text,text_formatting,link,linkname) VALUES (:note_section,:sequence,:time,:title,:teaser,:text,:text_formatting,:link,:linkname)");
        $dbr->bindParam(':note_section', $note_section, PDO::PARAM_STR);
