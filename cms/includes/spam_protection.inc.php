  ...
  if(isset($_POST['spam_protection_submit']))
   {
    ...
    $recaptcha_key_public = !empty($_POST['recaptcha_key_public']) ? $_POST['recaptcha_key_public'] : '';
    $recaptcha_key_private = !empty($_POST['recaptcha_key_private']) ? $_POST['recaptcha_key_private'] : '';
    ...
    if(empty($errors))
     {
      ...
      $dbr->execute();
      //added recaptcha
      $dbr->bindValue(':name', 'recaptcha_key_public', PDO::PARAM_STR);
      $dbr->bindParam(':value', $recaptcha_key_public, PDO::PARAM_STR);
      $dbr->execute();
      $dbr->bindValue(':name', 'recaptcha_key_private', PDO::PARAM_STR);
      $dbr->bindParam(':value', $recaptcha_key_private, PDO::PARAM_STR);
      $dbr->execute();
    ...
    //recaptcha  
     if(isset($_POST['recaptcha_key_public'])) $template->assign('recaptcha_key_public',htmlspecialchars(stripslashes($_POST['recaptcha_key_public'])));
     if(isset($_POST['recaptcha_key_private'])) $template->assign('recaptcha_key_private',htmlspecialchars(stripslashes($_POST['recaptcha_key_private'])));
  ...
  else
   {
    ...
    $template->assign('recaptcha_key_public',htmlspecialchars(stripslashes($settings['recaptcha_key_public'])));
    $template->assign('recaptcha_key_private',htmlspecialchars(stripslashes($settings['recaptcha_key_private'])));
