...
if(isset($_GET['get_1'])) // note opened
 {
  $dbr = Database::$content->prepare("SELECT id, note_section, time, title, teaser, text, text_formatting FROM ".Database::$db_settings['notes_table']." WHERE id=:id LIMIT 1");
  ...
  if(isset($note_data['id']) && $note_data['note_section']==$data['type_addition'])
   {
    ...
    $note['teaser'] = htmlspecialchars($note_data['teaser']);
...
else // overview
 {
  ...
  $dbr = Database::$content->prepare("SELECT id, time, title, teaser, text, text_formatting, link, linkname FROM ".Database::$db_settings['notes_table']." WHERE note_section=:note_section ORDER BY sequence ASC LIMIT ".(($current_page-1)*$settings['notes_per_page']).", ".$settings['notes_per_page']);
  $dbr->bindParam(':note_section', $data['type_addition'], PDO::PARAM_STR);
  $dbr->execute();
  $i=0;
  while($notes_data = $dbr->fetch())
   {
    ...
    $notes[$i]['teaser'] = $notes_data['teaser'];
