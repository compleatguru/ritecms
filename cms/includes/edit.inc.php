  ...
  if(isset($_GET['id']))
   {
    ...
    elseif(!is_authorized_to_edit($_SESSION[$settings['session_prefix'].'user_id'],$_SESSION[$settings['session_prefix'].'user_type'],$data['author'],$data['edit_permission'],$data['edit_permission_general']))
    ...
    else
     {
      ...
      $page_data['gcb_1'] = intval($data['gcb_1']);
      $page_data['gcb_2'] = intval($data['gcb_2']);
      $page_data['gcb_3'] = intval($data['gcb_3']);
      $page_data['template'] = htmlspecialchars($data['template']);
      $page_data['template_mobile'] = htmlspecialchars($data['template_mobile']);
      $page_data['language_file'] = htmlspecialchars($data['language_file']);
      ...
      $page_data['inline_css'] = htmlspecialchars($data['inline_css']);
      ...
      $page_data['custom_values'] = str_replace(',',', ',htmlspecialchars($data['custom_values']));
  ...
  else
   {
    ...
    $page_data['template_mobile'] = $settings['default_template_mobile'];
  ...
  if(isset($_POST['content']))
   {
    ...
    if(empty($errors))
     {
      ...
      $_POST['gcb_1'] = isset($_POST['gcb_1']) ? intval($_POST['gcb_1']) : 0;
      $_POST['gcb_2'] = isset($_POST['gcb_2']) ? intval($_POST['gcb_2']) : 0;
      $_POST['gcb_3'] = isset($_POST['gcb_3']) ? intval($_POST['gcb_3']) : 0;
      ...
      // trim custom values:
      $custom_values_array = explode(',',$_POST['custom_values']);
      foreach($custom_values_array as $item)
       {
        if(trim($item)!='')
         {
          $cleared_custom_values_array[] = trim($item);
         }
       }
      $_POST['custom_values'] = '';
      if(isset($cleared_custom_values_array))
       {
        $cleared_custom_values_array_count = count($cleared_custom_values_array);
        $i=1;
        foreach($cleared_custom_values_array as $custom_value)
         {
          $_POST['custom_values'] .= $custom_value;
          if($i<$cleared_custom_values_array_count) $_POST['custom_values'] .= ',';
          ++$i;
         }
       }
    ...
    if(empty($errors))
      ...
      if(isset($_POST['id']) && empty($_POST['edit_mode']))
       {
        ...
        $dbr->bindParam(28, $_POST['inline_css'], PDO::PARAM_STR);
        $dbr->bindParam(29, $_POST['sections'], PDO::PARAM_STR);
        $dbr->bindParam(30, $_POST['include_page'], PDO::PARAM_INT);
        $dbr->bindParam(31, $_POST['include_order'], PDO::PARAM_INT);
        $dbr->bindParam(32, $_POST['include_rss'], PDO::PARAM_INT);
        $dbr->bindParam(33, $_POST['include_sitemap'], PDO::PARAM_INT);
        $dbr->bindParam(34, $_POST['include_news'], PDO::PARAM_INT);
        $dbr->bindParam(35, $_POST['link_name'], PDO::PARAM_STR);
        $dbr->bindParam(36, $_POST['menu_1'], PDO::PARAM_STR);
        $dbr->bindParam(37, $_POST['menu_2'], PDO::PARAM_STR);
        $dbr->bindParam(38, $_POST['menu_3'], PDO::PARAM_STR);
        $dbr->bindParam(39, $_POST['gcb_1'], PDO::PARAM_INT);
        $dbr->bindParam(40, $_POST['gcb_2'], PDO::PARAM_INT);
        $dbr->bindParam(41, $_POST['gcb_3'], PDO::PARAM_INT);
        $dbr->bindParam(42, $_POST['template'], PDO::PARAM_STR);
        $dbr->bindParam(43, $_POST['template_mobile'], PDO::PARAM_STR);
        $dbr->bindParam(44, $_POST['language_file'], PDO::PARAM_STR);
        $dbr->bindParam(45, $_POST['content_type'], PDO::PARAM_STR);
        $dbr->bindParam(46, $_POST['charset'], PDO::PARAM_STR);
        $dbr->bindParam(47, $_POST['page_notes'], PDO::PARAM_STR);
        $dbr->bindParam(48, $edit_permission_list, PDO::PARAM_STR);
        $dbr->bindParam(49, $_POST['edit_permission_general'], PDO::PARAM_INT);
        $dbr->bindParam(50, $_POST['custom_values'], PDO::PARAM_STR);
        $dbr->bindParam(51, $_POST['status'], PDO::PARAM_INT);
        $dbr->bindParam(52, $_POST['id'], PDO::PARAM_INT);
        $dbr->execute();
       }
      else
       {
        ...
        $dbr->bindParam(50, $_POST['custom_values'], PDO::PARAM_STR);
        $dbr->bindParam(51, $_POST['status'], PDO::PARAM_INT);
        $dbr->bindParam(52, $_SESSION[$settings['session_prefix'].'user_id'], PDO::PARAM_INT);
        $dbr->execute();
       }
    ...
    else
     {
      ...
      $page_data['inline_css'] = isset($_POST['inline_css']) ? htmlspecialchars($_POST['inline_css']) : '';
      ...
      $page_data['template_mobile'] = isset($_POST['template_mobile']) ? htmlspecialchars($_POST['template_mobile']) : $settings['default_template_mobile'];
      $page_data['language_file'] = isset($_POST['language_file']) ? htmlspecialchars($_POST['language_file']) : '';
      ...
      $page_data['gcb_1'] = isset($_POST['gcb_1']) ? intval($_POST['gcb_1']) : 0;
      $page_data['gcb_2'] = isset($_POST['gcb_2']) ? intval($_POST['gcb_2']) : 0;
      $page_data['gcb_3'] = isset($_POST['gcb_3']) ? intval($_POST['gcb_3']) : 0;
      ...
      $page_data['custom_values'] = isset($_POST['custom_values']) ? htmlspecialchars($_POST['custom_values']) : '';
  ...
  switch($action)
   {
    case 'main':
     ...
     $gcb_result = Database::$content->query("SELECT id, title FROM ".Database::$db_settings['gcb_table']." ORDER BY id ASC");
     $i=0;
     while($gcb_data = $gcb_result->fetch())
      {
       $gcbs[$i]['id'] = $gcb_data['id'];
       $gcbs[$i]['title'] = $gcb_data['title'];
       $i++;
      }
     if(isset($gcbs))
      {
     ...
     // get available page language files:
     $handle=opendir('./lang/');
     while($file = readdir($handle))
      {
       if(preg_match('/(?<!\.admin)\.lang\.php$/i', $file))
        {
         $language_file_array[] = $file;
        }
      }
     closedir($handle);
     natcasesort($language_file_array);
     $i=0;
     foreach($language_file_array as $file)
      {
       $page_language_files[$i]['file'] = $file;
       $page_language_files[$i]['name'] = str_replace('.lang.php','',$file);
       $name_parts = explode('.', $page_language_files[$i]['name']);
       if(isset($name_parts[1])) $page_language_files[$i]['name'] = $name_parts[0].' ('.$name_parts[1].')';

       $i++;
      }
     if(isset($page_language_files))
      {
       $template->assign('page_language_files',$page_language_files);
      }
